...
export class AuthRoute extends BaseRoute {
  ...
  protected initializeRoutes(): void {
    this.router.post('/signup', asyncHandler(authController.signup.bind(authController)));
    this.router.post('/otp', asyncHandler(authController.sendOtp.bind(authController)));

    this.router.patch('/verify', asyncHandler(authController.verifyOtp.bind(authController)));
    ...
    this.router.post('/refresh', asyncHandler(authController.tokenRefresh.bind(authController)));
    ...
    this.router.get('/github', githubAuthController.redirectToGithub.bind(githubAuthController));

    this.router.get(
      '/google',
      googleAuthService.getPassport().authenticate('google', { scope: ['profile', 'email'] })
    );

    this.router.get(
      '/google/callback',
      googleAuthService.getPassport().authenticate('google', {
        session: false,
    ...
    this.router.post('/admin/login', asyncHandler(authController.adminLogin.bind(authController)));
