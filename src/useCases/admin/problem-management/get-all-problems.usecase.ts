...
import { IGetAllProblemsUsecase } from '../../Interfaces/admin/problem-management/get-all-problem.usecase.interface';
import { getAllProblemsUsecaseMapper } from '../../dtos/mappers/mappers';
...
export class GetAllProblemsUsecase implements IGetAllProblemsUsecase {
  ...
  async execute(data: IGetAllProblemUsecaseInputDto): Promise<IGetAllProblemUsecaseOutputDto> {
    const sort: Sort = PROBLEM_SORTING[data.sortBy] || PROBLEM_SORTING.NEWEST;
    const filter: GenericFilter = data.search
