...
import { IForgetPasswordUsecase } from '../Interfaces/auth/forget-password.usecase.interface';
...
export class ForgetPasswordUsecase implements IForgetPasswordUsecase {
  ...
  async execute(token: string, newPassword: string): Promise<void> {
    ...
    if (!accountId) {
      throw new CustomError(HTTP_STATUS.BAD_REQUEST, ERROR_MESSAGES.REST_LINK_EXPIRE);
    }
