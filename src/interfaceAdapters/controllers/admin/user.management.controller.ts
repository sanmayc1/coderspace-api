...
import { IGetUsersUsecase } from '../../../useCases/Interfaces/admin/user-management/get-users.usecase.interface';
import {
  commonResponse,
  CustomError,
  ERROR_MESSAGES,
...
import { searchRegex } from '../../../shared/validation/regex';
import { UserProfileUpdateSchema } from './validation/schema';
import { IUpdateUserUsecase } from '../../../useCases/Interfaces/admin/user-management/update-user.usecase.interface';
import { IUpdateUserStatusUsecase } from '../../../useCases/Interfaces/admin/user-management/update-user-status.usecase.interface';
...
export class UserManagementController {
  ...
  async getAllUsers(req: Request, res: Response) {
    ...
    if (Number.isNaN(limitNumber) || limitNumber <= 0) {
      throw new CustomError(HTTP_STATUS.BAD_REQUEST, ERROR_MESSAGES.LIMIT_NOT_NUMBER);
    }

    if (sortValue && !USER_SORTING.hasOwnProperty(sortValue)) {
      throw new CustomError(HTTP_STATUS.BAD_REQUEST, ERROR_MESSAGES.INVALID_SORT);
    }

    if (searchValue && !searchRegex.test(searchValue)) {
      throw new CustomError(HTTP_STATUS.BAD_REQUEST, ERROR_MESSAGES.INVALID_SEARCH);
    }
  ...
  async updateUserProfile(req: Request, res: Response) {
    ...
    if (!validated.success) {
      throw new CustomError(HTTP_STATUS.BAD_REQUEST, validated.error?._zod.def[0].message);
    }
