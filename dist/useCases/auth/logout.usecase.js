"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LogoutUsecase = void 0;
const tsyringe_1 = require("tsyringe");
let LogoutUsecase = class LogoutUsecase {
    _blacklistRepo;
    _jwtService;
    constructor(_blacklistRepo, _jwtService) {
        this._blacklistRepo = _blacklistRepo;
        this._jwtService = _jwtService;
    }
    async executes(refreshToken, accessToken) {
        const accessPayload = this._jwtService.verifyAccess(accessToken);
        const refreshPayload = this._jwtService.verifyAccess(refreshToken);
        const now = Math.floor(Date.now() / 1000);
        const accessExpire = accessPayload?.exp ? Math.max(accessPayload.exp - now, 0) : 0;
        const refreshExpire = refreshPayload?.exp ? Math.max(refreshPayload.exp - now, 0) : 0;
        if (accessExpire > 0) {
            await this._blacklistRepo.save(`blacklist:${accessToken}`, accessExpire);
        }
        if (refreshExpire > 0) {
            await this._blacklistRepo.save(`blacklist:${refreshToken}`, refreshExpire);
        }
    }
};
exports.LogoutUsecase = LogoutUsecase;
exports.LogoutUsecase = LogoutUsecase = __decorate([
    (0, tsyringe_1.injectable)(),
    __param(0, (0, tsyringe_1.inject)('IBlackListTokenRepository')),
    __param(1, (0, tsyringe_1.inject)('IJwtService')),
    __metadata("design:paramtypes", [Object, Object])
], LogoutUsecase);
//# sourceMappingURL=logout.usecase.js.map