...
let AuthUserUsecase = class AuthUserUsecase {
    _userRepository;
    _accountRepository;
    constructor(_userRepository, _accountRepository) {
    ...
    async execute(user) {
        ...
        if (user.role === 'user') {
            const userProfile = await this._userRepository.findByAccountId(account._id);
            if (!userProfile) {
                throw new custom_error_1.CustomError(constant_1.HTTP_STATUS.BAD_REQUEST, constant_1.ERROR_MESSAGES.ACCOUNT_NOT_FOUND);
            }
            const response = mappers_1.authUserUsecaseMapper.toOutput(account, userProfile);
            return response;
        }
        const response = mappers_1.authUserUsecaseMapper.toOutput(account);
        return response;
    }
};
...
//# sourceMappingURL=auth-user.usecase.js.map